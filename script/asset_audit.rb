require 'bundler'
Bundler.setup(:default, :asset_audit)
require 'mechanize'
require 'csv'

email = ENV.fetch('EMAIL')
password = ENV.fetch('PASSWORD')
seed = ENV.fetch('SEED', 123)
number_of_samples = ENV.fetch('NUMBER_OF_SAMPLES', 1)
...
samples = AttachmentData.order("RAND(#{seed})").limit(number_of_samples)
